...
export default function CreatePage() {
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="bg-white border border-gray-200 rounded-2xl p-8 w-full max-w-md shadow-sm flex flex-col gap-5 text-center">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => { setCreatedCode(null); setTitle(""); setQuestions([{ question: "", answer: "" }]); setCurrent(0); }}
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-10 gap-4">

      {/* Jedna velika bela kartica */}
      <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-md shadow-sm flex flex-col overflow-hidden">
        ...
        <div className="px-6 pt-5 pb-4 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <a href="/" className="text-gray-500 hover:text-gray-800 text-sm font-medium transition">← Nazad</a>
            <h1 className="text-gray-800 font-semibold text-lg">✏️ Kreiraj kviz</h1>
            <span className="text-gray-500 text-sm font-semibold">{current + 1} / {questions.length}</span>
          </div>
        ...
        <div className="px-6 py-5 flex flex-col gap-2">
          <label className="text-sm text-gray-600 font-semibold">Naziv kviza</label>
          <input
            className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm bg-gray-50 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300"
            placeholder="npr. Opšte znanje, Geografija..."
            value={title}
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <label className="text-sm text-gray-600 font-semibold">Pitanje {current + 1}</label>
            {questions.length > 1 && (
              <button onClick={removeCurrentQuestion} className="text-xs text-red-400 hover:text-red-600 transition">
                Ukloni
              </button>
            )}
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={current === 0}
              className="flex-1 py-2 border border-gray-200 text-gray-500 rounded-xl text-sm hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition"
            >
              ← Prethodno
            </button>
            <button
        ...
        {/* Kreiraj kviz dugme — unutar kartice, centrirano */}
        <div className="px-6 pb-6">
          <button
            onClick={createQuiz}
            className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-xl text-sm transition flex items-center justify-center gap-2"
          >
            Kreiraj kviz
            {filledCount > 0 && (
